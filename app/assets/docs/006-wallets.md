...
`sumcash-qt` provides a combination full Sumcash network client and wallet. Sumcash-qt is highly portable application written in QT5 framework.
...
`sumcash-cli` allows you to send JSON-RPC commands to running instance of `sumcashd` from the command line.
For example:
> sumcash-cli help

> sumcash-cli getinfo

All three programs get settings from sumcash.conf in the Sumcash application directory:

> Windows: %APPDATA%\Sumcash\

> OSX: $HOME/Library/Application Support/Sumcash/

>Linux: $HOME/.sumcash/

To use `sumcashd` and `sumcash-cli`, you will need to add a RPC password to your sumcash.conf file. Both programs will read from the same file if both run on the same system as the same user, so any long random password will work:

> rpcpassword=change_this_to_a_long_random_password

You should also make the sumcash.conf file only readable to its owner.
On Linux, Mac OSX, and other Unix-like systems, this can be accomplished by running the following command in the Sumcash application directory:

> chmod 0600 sumcash.conf
...
As of April 2018, Sumcash has official Debian repository hosted at https://sumcash.github.io/deb-repo/.
Repository is serving .deb packages for latest Debian stable, for amd64, arm64 and armhf hardware architectures.

#### Adding the GPG key

`wget -O - https://sumcash.github.io/deb-repo/sumcash.apt.key | sudo apt-key add -`

#### Adding the repository in the sources

`sudo sh -c "echo 'deb https://sumcash.github.io/deb-repo/ buster main' >> /etc/apt/sources.list.d/sumcash.list"`

`sudo apt update`

#### Installing the packages

`sudo apt install sumcash-qt`
...
From this tab you can see the "Available", "Pending", and "Total" balance of your Sumcash wallet.  On the right side, you can see your "Recent transactions" as well which will show the most recent transactions that have come into the wallet recently.

### Send

![Send Tab](../img/send.png)

The send tab is used to send Sumcash.  The "Pay To" section is where a target address can be entered.  If you wish to label the address for future use, such as an exchange, you may add it to your address book.  Amount determines the quantity of Sumcash which will be sent in the transaction.  If the wallet is encrypted, the secure password must be entered before the send transaction can be finalized.
...
![Sumcash staking](../img/staking.png)
...
If you are interested in calculating the rough time until minting takes place, you can use this calculator: http://poscalculator.sumcashtalk.org/
...
If you want to send Sumcash, navigate to the "Send" tab.  We are going to send some Sumcashs to the address we created above.  This page has three fields: "Pay To", "Label", and "Amount".  Paste the address you want to send Sumcashs to into the "Pay To" field.  Make sure to verify that this address is correct.  If you already have a label for this wallet, it will automatically fill in the "Label" field.  If not, you can write your own in and it will be saved in your address book (Under the "Address" tab).  In the amount field, insert the number of Sumcashs you would like to send.  There is a drop down menu which allows you to determine if you want to send in denominations of "Sumcashs", "MilliSumcashs", "MicroSumcashs".  Make sure that you have enough to pay for the transaction fee.  When everything is finalized, press the "Send" button to confirm the transaction.  You will be prompted to confirm the transaction and must enter your wallet password to verify the transaction.  Once this is completed, you can view the transaction under the "Transactions" tab.

![Sending Sumcash](../img/sendingsumcash.png)

_________________________________________

## Using the multisig

You can access multisig graphic interface in latest builds of Sumcash-qt. Open Sumcash-qt, click on File - Multisig. Now you see Multisig interface.

![Sumcash-qt multisig](../img/multisig.png)
...
Reference Sumcash client is not capable of indexing the multisig addresses and showing their balance because multisig addresses can be owned by keys which are not part of the wallet (friends, family, backup). Thus the procedure to spend the funds from the multisig is a bit more complicated, more "low level" then usual.

In the "Spend Funds" tab of the multisig dialog, on the left there is "Inputs" table. You need to provide with the [UTXO](https://en.wikipedia.org/wiki/Unspent_transaction_output) you want to spend. That is, the `txid` and output index. On the right side there is "Outputs" table, where the desired outputs will be placed. Please note that you have to include the change output and deduct the tx fee (0.01 SUMC per kB).
...
![Sumcash-qt multisig spending](../img/multisig-spend.png)
...
The transaction will have 360 SUMC (Alice's coins) as input and 359.99 SUMC as output (because of the mandatory 0.01 SUMC transaction fee; the transaction won't be accepted by the network without it).
...
## How To Sign a Message Using the Sumcash Client
...
Stakebox is a plug and play Pi with the Sumcash wallet pre-installed.What is need is a keyboard and monitor to plug in to and you are ready to go.
...
Beside ordering a pre-installed Stakebox from [PiSupply website](https://www.stakebox.org/products/sumcash-stakebox) you can also make one yourself.
...
### Ledger Sumcash Tutorial

#### Ledger Live

If you wish to store Sumcash on the Ledger Nano or Ledger Blue, this tutorial will guide you through using the Ledger Live program

Open the Ledger Live program and enter your password to unlock the wallet.  In order to store Sumcash, we first need to add the Sumcash wallet application to the Ledger device.  If this is the first time you are using a Ledger device, you will have to install the Bitcoin app first.  Follow the same steps to add the Bitcoin app, then do the same process for the Sumcash application.  Select Manager from the sidebar and make sure your device is connected, unlocked, and allows access when prompted by the Ledger Manager.

![Ledger Live main screen](../img/ledgerman_main.JPG)

Once this is done, you will be presented with a variety of apps that can be added.  Type in "Sumcash" to find the Sumcash app.  Click the "Install" button and you will be asked to confirm this on your Ledger device.  After a moment, the app will be done installing.  Click "Okay" to proceed.

![Manager](../img/ledgerman_manager.JPG)

![Installing](../img/ledgerman_installing.JPG)

Once this is finished, the Sumcash app is now installed on your Ledger device.  Now we need to add an actual wallet account which we can do by clicking "Portfolio" to return to our starting screen.  Once you are on the main screen, click the "+" by the Accounts tab to add a new asset.

![Ledger Live main screen](../img/ledgerlive_main.JPG)

This will open the menu where you can search for the asset account you would like to add.  Input "Sumcash" and click continue.  You will have to unlock your Ledger again and navigate to the Sumcash app.  To open it, press both buttons on the Ledger Nano device simultaneously.  Once this is done, Ledger Live will sync and you will have the option to name the newly created account.

![Which Asset do you want to add?](../img/ledgerlive_select.JPG)

![Adding Sumcash](../img/ledgerlive_select2.JPG)

Click continue until everything is finalized and completed.  You now have the Sumcash app, and the account added to your Ledger product and wallet.

## Sumcash Paper Wallet Guide
...
*Paper Wallet (print)
   *This option will open a print dialogue from which the wallet can be printed or saved as a PDF.  Paper wallets provide physical security and backups for Sumcashs that are to be left in cold storage.*

This concludes the tutorial on the paper wallet generator.  This platform provides extra security for those who wish to keep their Sumcash in a safe and physical location.
